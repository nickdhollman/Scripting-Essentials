...
# check for directory mod08t, if not make directory
dircheck=~/mod08t
if ! [[ -d $dircheck ]];then
	mkdir ~/mod08t
fi
...
# convert spaces to underscore in employee string
employee_string=$(echo $employee_string | tr " " "_")
# replace commas in employee_string with a space
employee_string=$(echo $employee_string | tr "," " ")

# make employee_array from employee string deliminter of spaces
...
# i is the Name, i+1 is the Address, i+2 is the City
for (( i=0; i<$array_len; i=$(($i + 3)) ))
do
# make the line_out variable
	line_out="${employee_array[$i]},\
${employee_array[$(($i+1))]},\
${employee_array[$(($i+2))]}"
# write data out using line_out to employee.csv
...
do
	# cut the data on comma (since it is a csv)
	# take the third field (which is the city)
	city=$(echo $line | cut -d "," -f3)
	# because we don't have a command to skip headers
	# we are saying below to skip "City" which is the header
	if ! [[ $city == "City" ]]; then
		city_array+=($city)
	fi
done < $input
...
#read through the employee.csv file
while IFS= read -r line
do
	...
	for i in ${!city_array[*]};
	do
		#while iterating through the city array and retrieve the first field
		city_from_array=$(echo ${city_array[$i]} | cut -d "|" -f 1)
		#if city matches the city array value
		#append the city name and append a pipe and the last name
...
for i in ${city_array[*]}
do
	# take city_array and throw it into a temporary array
	# take the pipes and turn them back into spaces and cut on spaces
	temp_array=($(echo $i | tr "|" " " | cut -d " " -f 1-))
	#city name: rest of elements in array replacing _ with spaces
	echo "${temp_array[0]}: ${temp_array[*]:1}" | tr "_" " "

done

#pack up employee_data.csv into mod08t directory
cd ~/mod08t
#if we had other files we would just put a space after employee.csv and add them
