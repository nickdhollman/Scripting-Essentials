...
# substitue \ for a new line \n but you must escape the new line with another \
...
# convert each song to lower case
song1=$(echo $song1 | tr [:upper:] [:lower:])
song2=$(echo $song2 | tr [:upper:] [:lower:])

# substitute new line for a space and do it globally
song1=$(echo $song1 | sed 's/\\n/ /g')
song2=$(echo $song2 | sed 's/\\n/ /g')

# cutting on the delimiter (-d) of space " ", getting fields (-f) one onward (1-)
song1_array=($(echo -e $song1 | cut -d " " -f 1-))
song2_array=($(echo -e $song2 | cut -d " " -f 1-))

# iterate through song1
# for every element we pull out of song 1 array (i is word in song 1)
for i in ${song1_array[*]};
do
    # we will iterate through song 2 array (j is word in song 2)
    for j in ${song2_array[*]};
    do
        # stuff duplicates words into duplicate object
        if [[ $j == $i ]];then
           duplicates+=("$j ")
        fi
    done
...
# sort will sort duplicates, -u will only give the unique values, ; allows you to have multiple commands on one line
